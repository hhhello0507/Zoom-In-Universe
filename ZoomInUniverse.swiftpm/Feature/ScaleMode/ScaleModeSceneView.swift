...
struct ScaleModeSceneView: UIViewRepresentable, ScaleModeSceneViewProtocol {
    ...
    func setupScene() {
        let objects = [
            ...
            molecule,
            SCNNode().apply {
                $0.addChildNodes(dna, cell)
            },
            spaceBackground,
            SCNNode().apply {
                $0.addChildNodes(phone, computer, lightBulb)
            },
            human,
            pyramid,
            ocean,
            earthNode,
            SCNNode().apply {
                $0.addChildNodes(clouds, airplain)
            },
            SCNNode().apply {
                $0.addChildNodes(aurora, artificialSatellite)
            },
            SCNNode().apply {
                $0.addChildNodes(mercury, venus, moon, mars)
            },
            __START_NODE__,
            SCNNode().apply {
                $0.addChildNodes(jupiter, saturn, uranus, neptune)
            },
            cosmicWebBackground,
            solarSystem,
